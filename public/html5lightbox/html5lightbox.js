...
function loadHtml5LightBox(jsFolder) {
    (function ($) {
        $.fn.html5lightbox = function (options) {
            ...
            var ELEM_TYPE = 0, ELEM_HREF = 1, ELEM_TITLE = 2, ELEM_GROUP = 3, ELEM_WIDTH = 4, ELEM_HEIGHT = 5, ELEM_HREF_WEBM = 6, ELEM_HREF_OGG = 7, ELEM_THUMBNAIL = 8, ELEM_DESCRIPTION = 9,
                ELEM_DIV = 10, ELEM_ORIGINALWIDTH = 11, ELEM_ORIGINALHEIGHT =
                    12, ELEM_SOCIALMEDIA = 13, ELEM_WEBLINK = 14, ELEM_WEBLINKTARGET = 15, ELEM_WEBLINKTEXT = 16, ELEM_MEDIUMWIDTH = 17, ELEM_MEDIUMHEIGHT = 18, ELEM_SMALLWIDTH = 19,
                ELEM_SMALLHEIGHT = 20;
                    ...
                    if (nodeValue && nodeValue.attr("class")) {
                        ...
                        $.each(classList, function (index, className) {
                            if (className && className.toLowerCase().indexOf("html5lightbox-" + attr + "-") == 0) {
                                ...
                                if (names.length == 3) {
                                    var val = names[2];
                                    if (val.toLowerCase() == "true") val = true; else if (val.toLowerCase() == "false") val = false; else if (/^\d+$/.test(val)) val = parseInt(val); else if (!isNaN(val)) val = parseFloat(val);
                                    ret = val
                                }
                            }
                        })
            ...
            inst.readNodeData =
                function (node, parentNode) {
                    ...
                    for (var i = 0; i < inst.elemArray.length; i++) if (node.attr("href") ==
            ...
            inst.createMarkup = function () {
                ...
                if (inst.options.showsocial) {
                    ...
                    $(".html5-social-btn", inst.$lightbox).click(function () {
                        ...
                        if ($(this).hasClass("html5-social-facebook")) window.open("https://www.facebook.com/sharer/sharer.php?u=" +
                            ...
                            "true" : "false"), "_blank"); else if ($(this).hasClass("html5-social-email")) window.open("mailto:?subject=" + encodeURIComponent(shareTitle) + "&body=" + inst.options.sharetextprefix + encodeURIComponent(shareUrl));
                        return false
                    })
                }
                ...
                if (inst.options.closeusefont && (inst.options.fullscreenmode || inst.options.closepos == "topright" || inst.options.closepos ==
                    "inside")) inst.$close.append('<div class="mh-icon-close"></div>'); else inst.$close.append("<img alt='' src='" + (inst.options.fullscreenmode || inst.options.closepos == "topright" ? inst.options.fullscreencloseimage : inst.options.closeimage) + "'>");
            ...
            inst.calcNextPrevElem = function () {
                ...
                if (curGroup != undefined && curGroup != null) {
                    for (j = inst.options.curElem + 1; j < inst.elemArray.length; j++) if (inst.matchGroup(curGroup,
            ...
            inst.showNavigation = function () {
                ...
                if (inst.options.shownavcontrol) {
                    ...
                    if (inst.options.navusefont) $(".html5-nav-showcontrol").append('<div class="mh-icon-th-thumb"></div>'); else $(".html5-nav-showcontrol").append('<img alt="" src="' + inst.options.navcontrolimage + '"></img>');
            ...
            inst.showTitle = function (t, description) {
                ...
                if (elem[ELEM_TYPE] == 0) {
                    if (elem[wIndex]) elemW = elem[wIndex]; else if (initW) {
                        elemW = initW;
                        elem[wIndex] = initW
                    } else elemW = inst.options.defaultwidth;
                    if (elem[hIndex]) elemH = elem[hIndex]; else if (initH) {
                        elemH = initH;
                        elem[hIndex] = initH
                ...
                inst.resizeLightbox(dataW, dataH, true, function () {
                    ...
                    if (isHTML5) {
                        ...
                        if (elem[ELEM_WEBLINK] && elem[ELEM_WEBLINK].length > 0) $(".html5-lightbox-video",
                            inst.$image).css({cursor: "pointer"}).click(function () {
            ...
            inst.resizeWindow = function () {
                ...
                if (inst.options.autoresizecontent && (inst.currentElem[ELEM_TYPE] == 5 || inst.currentElem[ELEM_TYPE] == 7 || inst.currentElem[ELEM_TYPE] == 10)) {
                    ...
                    if (inst.currentElem[ELEM_TYPE] == 7 && $(".html5lightbox-web-iframe", inst.$lightbox).length > 0 && $(".html5lightbox-web-iframe",
                        ...
                        if (iframe && iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document.documentElement.offsetHeight) if (elemSize.h > iframe.contentWindow.document.documentElement.offsetHeight) {
            ...
            inst.resizeLightbox = function (elemW, elemH, bAnimate, onFinish, speedZero) {
                ...
                if (!inst.options.fullscreenmode && !(inst.options.navarrowspos ==
                    "side") && !(inst.options.navarrowspos == "browserside") && (!inst.options.isTouch || !inst.options.navarrowsalwaysshowontouch) && !inst.options.alwaysshownavarrows) {
                    inst.$lightboxBox.on("mouseenter mousemove", function () {
                        if (inst.options.arrowloop && inst.options.prevElem >= 0 || !inst.options.arrowloop && inst.options.prevElem >= 0 && inst.options.prevElem < inst.options.curElem) inst.$prev.fadeIn();
                        if (inst.options.arrowloop && inst.options.nextElem >= 0 || !inst.options.arrowloop && inst.options.nextElem >= 0 && inst.options.nextElem >
                            inst.options.curElem) inst.$next.fadeIn()
                    });
            ...
            inst.finish = function () {
                ...
                if (inst.options.exitanimation == "fadeOut" || inst.options.exitanimation ==
                    "fadeOutDown") {
            ...
            inst.enableSwipe = function () {
                ...
                inst.$lightboxBox.html5lightboxTouchSwipe({
                    preventWebBrowser: preventDefault, swipeDistance: inst.options.swipedistance, swipeLeft: function () {
            ...
            inst.embedHTML5Video = function ($container, src, autoplay, loopvideo, mutevideo, playsinline) {
                $container.html("<div style='display:block;width:100%;height:100%;position:relative;'><video class='html5-lightbox-video' width='100%' height='100%'" + (inst.options.html5videoposter && inst.options.html5videoposter.length > 0 ? "poster='" + inst.options.html5videoposter +
        ...
        $.fn.lightboxHTML5VideoControls = function (skinFolder, parentInst, videoElem, hideControls, hidePlayButton, defaultVolume, fullscreenNativeControls, html5VideoNoDownload, skinImages, showsubtitle, vttline, showsubtitlebydefault) {
            ...
            if ($videoObj.get(0).volume === volume / 2 + 0.1) {
                ...
                $videoVolumeBarBg.css({
                    display: "block",
                    position: "relative", width: 10, height: 68, margin: 7, cursor: "pointer", "background-color": "#222"
                });
            ...
            var setVttline = function (vttline) {
                if ($videoObj.get(0).textTracks) {
                    for (var i = 0; i < $videoObj.get(0).textTracks.length; i++) {
                        ...
                        if ($videoObj.get(0).textTracks[i].activeCues) for (var j =
    ...
    jQuery(document).ready(function () {
        if (typeof html5Lightbox === "undefined") if (jQuery.fn.html5lightbox) html5Lightbox =
